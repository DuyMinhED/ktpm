...
public class ClinicDashboardControllerTest {
    ...
    void updateAppointment_success() throws Exception {
        Long appointmentId = 9L;
        DoctorCreateAppointmentRequest request = new DoctorCreateAppointmentRequest();
        request.setPatientId(1L);
        request.setAppointmentDate("2026-05-28");
        request.setAppointmentTime("11:00");
        request.setType("ONLINE");

        doNothing().when(clinicDashboardService)
                .updateAppointment(eq(clinicId), eq(appointmentId), any(DoctorCreateAppointmentRequest.class));

        mockMvc.perform(put("/api/v1/clinics/{clinicId}/appointments/{appointmentId}", clinicId, appointmentId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        verify(clinicDashboardService).updateAppointment(eq(clinicId), eq(appointmentId),
                any(DoctorCreateAppointmentRequest.class));
    }
    ...
    void updateAppointmentStatus_success() throws Exception {
        ...
        doNothing().when(clinicDashboardService).updateAppointmentStatus(clinicId, appointmentId, "COMPLETED");

        mockMvc.perform(patch("/api/v1/clinics/{clinicId}/appointments/{appointmentId}/status", clinicId, appointmentId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        verify(clinicDashboardService).updateAppointmentStatus(clinicId, appointmentId, "COMPLETED");
    }
