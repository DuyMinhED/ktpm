            ...
            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
              <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-slate-500 font-medium mt-0.5">{formatDate(appointment.appointmentTime)}</p>
                  </div>
              </div>
            ...
            <div className="px-8 py-6 space-y-6">
               ...
               <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                   <div className="size-12 rounded-full overflow-hidden ring-2 ring-white dark:ring-slate-700 shadow-sm">
                        <img src={appointment.doctorAvatarUrl || "https://ui-avatars.com/api/?name=" + encodeURIComponent(appointment.doctorName || 'Dr')} className="size-full object-cover" />
                   </div>
                   <div>
                       <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Bác sĩ phụ trách</p>
                       <h4 className="text-[15px] font-bold text-slate-900 dark:text-white">{appointment.doctorName}</h4>
                   </div>
               </div>
               ...
               <div className="space-y-2">
                   ...
                   <div className="w-full min-h-[120px] p-5 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-100 dark:border-slate-800 text-[15px] leading-relaxed text-slate-700 dark:text-slate-300 shadow-inner whitespace-pre-wrap">
                       {appointment.diagnosisSummary || "Bác sĩ chưa cập nhật chẩn đoán chi tiết cho buổi khám này."}
                   </div>
               </div>
               ...
               <div className="grid grid-cols-2 gap-4">
                   <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border border-blue-100 dark:border-blue-800/30">
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                            <span className="material-symbols-outlined text-[18px] text-blue-500">{appointment.appointmentType === 'ONLINE' ? 'video_call' : 'location_on'}</span>
                            <span className="text-[14px] font-bold text-slate-800 dark:text-slate-200">{appointment.appointmentType === 'ONLINE' ? 'Trực tuyến' : 'Tại phòng khám'}</span>
                        </div>
                   </div>
