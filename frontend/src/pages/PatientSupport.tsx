...
const PatientTicketDetailModal = ({ isOpen, onClose, ticket }: any) => {
    useEffect(() => {
        if (isOpen) {
    ...
    const fetchCreatorTickets = async () => {
        ...
        try {
            ...
            const mapped = (response.data.content || []).map((t: any) => ({
                ...
                dbId: t.id,
                user: t.creator?.fullName || 'Bệnh nhân',
                subject: t.subject,
                message: t.message,
            ...
            console.error("Failed to fetch patient tickets:", error);
